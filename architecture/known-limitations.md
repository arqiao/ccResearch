...
联通云智电脑是"云桌面"（VDI）产品，不是传统云服务器。云主机的生命周期绑定在客户端连接上：
- 客户端未连接时，云主机处于休眠/关机状态，所有服务不可用
- 没有 Web 控制台、没有 API、没有 Wake-on-LAN，无法远程唤醒
- 必须通过联通专属客户端手动连接，云主机才会开机

**影响**：云船掉线（断电、平台维护、客户端断开）后，必须有人手动打开客户端才能恢复服务。这是当前架构的单点故障。

### 9.2 启动后服务可能不完整

即使 Windows 自动登录 + 启动文件夹触发了 start-openclaw.bat，个别服务仍可能启动失败（路径错误、端口冲突、依赖服务未就绪等）。

**缓解措施**：
- startup-check.sh：启动后自检 + 飞书通知，手机上即可确认服务状态
- service-watchdog.sh：cron 每 10 分钟检测关键服务，异常时自动重启 + 飞书通知
